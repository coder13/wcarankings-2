...
CREATE TABLE IF NOT EXISTS lists (
  ...
  CONSTRAINT fk_lists_owner FOREIGN KEY (owner_user_id) REFERENCES app_users (id) ON DELETE CASCADE,
  CONSTRAINT chk_lists_identity CHECK (
    (
      kind = 'user'
      AND public_id IS NOT NULL
      AND system_alias IS NULL
...
CREATE TABLE IF NOT EXISTS list_members (
  ...
  CONSTRAINT fk_list_members_added_by FOREIGN KEY (added_by_user_id) REFERENCES app_users (id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS list_membership_requests (
  ...
  CONSTRAINT fk_list_requests_list FOREIGN KEY (list_id) REFERENCES lists (id) ON DELETE CASCADE,
  CONSTRAINT fk_list_requests_requester FOREIGN KEY (requester_user_id) REFERENCES app_users (id) ON DELETE CASCADE,
  CONSTRAINT fk_list_requests_resolver FOREIGN KEY (resolved_by_user_id) REFERENCES app_users (id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS list_exclusions (
  ...
  created_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP (6),
  PRIMARY KEY (list_id, person_id),
  CONSTRAINT fk_list_exclusions_list FOREIGN KEY (list_id) REFERENCES lists (id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS list_activity_events (
  ...
  created_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP (6),
  CONSTRAINT fk_list_activity_list FOREIGN KEY (list_id) REFERENCES lists (id) ON DELETE SET NULL,
  CONSTRAINT fk_list_activity_actor FOREIGN KEY (actor_user_id) REFERENCES app_users (id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS list_import_jobs (
  ...
  CONSTRAINT fk_list_import_jobs_list FOREIGN KEY (list_id) REFERENCES lists (id) ON DELETE CASCADE,
  CONSTRAINT fk_list_import_jobs_owner FOREIGN KEY (owner_user_id) REFERENCES app_users (id) ON DELETE CASCADE,
...
VALUES
  (
    ...
    'People named Max',
    'max',
    'People whose primary WCA display name starts with the exact token Max.',
    'public'
  ),
  (
    'system',
    ...
    'People named Luke',
    'luke',
    'People whose primary WCA display name starts with the exact token Luke.',
    'public'
  )
