RENAME TABLE
  list_ranking_cache_scopes TO list_person_ranking_cache_scopes,
...
ALTER TABLE list_ranking_cache_versions
  ...
  ADD INDEX idx_list_ranking_cache_ready_stream
    (target_key, grain, filter_key, membership_version, rankings_data_version, status, activated_at);

UPDATE list_ranking_cache_versions SET filter_key = 'world||all' WHERE filter_key = 'world|all';
...
ALTER TABLE list_ranking_rebuild_jobs
  ...
  ADD INDEX idx_list_ranking_rebuild_claim_stream (available_at, leased_until, priority, grain, filter_key);

UPDATE list_ranking_rebuild_jobs SET filter_key = 'world||all' WHERE filter_key = 'world|all';
...
CREATE TABLE list_result_ranking_cache_entries (
  ...
  PRIMARY KEY (cache_version_id, event_id, result_type, result_id, attempt_number),
  ...
  INDEX idx_list_result_cache_page (cache_version_id, event_id, result_type, list_position),
