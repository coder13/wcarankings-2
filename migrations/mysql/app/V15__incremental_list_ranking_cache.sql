...
ALTER TABLE list_ranking_cache_versions
  ...
  ADD INDEX idx_list_ranking_cache_ready_target (target_key, membership_version, rankings_data_version, status, activated_at);

INSERT INTO list_ranking_cache_targets (target_key, list_id, target_kind, membership_version, member_count)
...
SELECT CONCAT('list:', member.list_id), member.person_id
FROM list_members member
JOIN lists list ON list.id = member.list_id AND list.deleted_at IS NULL;

UPDATE list_ranking_cache_versions
SET target_key = CONCAT('list:', list_id)
WHERE target_key = '' AND list_id IS NOT NULL;
