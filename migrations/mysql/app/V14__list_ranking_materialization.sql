CREATE TABLE IF NOT EXISTS list_ranking_cache_versions (
  ...
  UNIQUE KEY uq_list_ranking_cache_build (list_id, membership_version, rankings_data_version, build_token),
  INDEX idx_list_ranking_cache_ready (list_id, membership_version, rankings_data_version, status, activated_at)
);
...
CREATE TABLE IF NOT EXISTS list_ranking_cache_entries (
  ...
  PRIMARY KEY (cache_version_id, event_id, result_type, person_id),
  ...
  INDEX idx_list_ranking_cache_page (cache_version_id, event_id, result_type, list_position),
