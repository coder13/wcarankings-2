...
export function ListAddPeopleRail({ listId, onCancel, onAdded }: { listId: string; onCancel: () => void; onAdded?: () => void }) {
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); if (!value.trim()) { void commit(); return; } const ids = value.split(/[\s,]+/).filter(Boolean); if (ids.length > 1 || /^\d{4}[A-Za-z0-9]{4}\d{2}$/.test(value.trim())) { void commit([...selected, ...ids.map((personId) => ({ personId: personId.toUpperCase(), name: personId.toUpperCase(), avatarUrl: null }))]); return; } const person = entries[active]; if (person) void commit([...selected, person]); };
