...
import {
  TextDropdown,
  type TextDropdownOption,
} from "../Dropdown/TextDropdown";
import {
  EventPicker,
  type EventPickerOption,
} from "../EventPicker/EventPicker";
...
export const RankingsRail = forwardRef<
  HTMLDivElement,
  {
    children: ReactNode;
    className?: string;
...
>(
  (
    ...
    const shouldAnimateLayout =
      animateLayout || className.includes("Jump--listAdd");
    ...
    return (
      <MotionConfig reducedMotion="user">
        <motion.div
          ...
          layoutId={
            shouldAnimateLayout ? `rankings-rail-${direction}` : undefined
          }
          initial={false}
          animate={{
  ...
  return (
    ...
    >
      <button
        ...
        onClick={openSearch}
        aria-label={t("rankingsRail.search.search")}
        title={t("rankingsRail.search.searchWithShortcut")}
      >
        <SearchIcon />
      </button>
      <input
      ...
      >
        {findLoading || findPending ? (
          <span
            ...
            aria-label={t("rankingsRail.search.searching")}
          />
        ) : (
          status
        )}
      ...
      <button
        className="findClose"
        type="button"
        tabIndex={searchOpen || hasSearchText ? 0 : -1}
        onMouseDown={(event) => event.preventDefault()}
        onClick={dismissSearch}
        aria-label={t("rankingsRail.search.close")}
      >
        <CloseIcon />
      </button>
    </div>
...
export function RankingsControlsRail<T extends EventPickerOption>({
  ...
  return (
    <RankingsRail
      className={`Jump--rankings${showResultType ? "" : " Jump--withoutResultType"}${hemisphere ? " Jump--withHemisphere" : ""}${listAddAction ? " Jump--withListAdd" : ""}`}
      direction="up"
  ...
  const currentLabels = {
    up:
      currentPosition <= 5000
        ...
        : t("rankingsRail.pager.jumpUp", {
            distance: formatRankingNumber(5000),
          }),
    down:
      Number.isFinite(total) && currentPosition >= total - 5000
        ? t("rankingsRail.pager.jumpToEnd")
        : t("rankingsRail.pager.jumpDown", {
            distance: formatRankingNumber(5000),
          }),
  };
  const [pendingLabels, setPendingLabels] = useState<
    typeof currentLabels | null
  >(null);
  const labels = busy ? (pendingLabels ?? currentLabels) : currentLabels;
  const jumpUp = () => {
    if (busy) {
      onJumpToTop?.();
      return;
  ...
  return (
    ...
    >
      <div className="Jump-pagerActions" aria-hidden={searchActive}>
        ...
        {wcaId && onFocusMe && (
          <button
            className="Jump-pagerButton Jump-pagerButton--me"
            onClick={() => onFocusMe(wcaId)}
            type="button"
            disabled={searchActive || busy}
            aria-label={t("rankingsRail.pager.jumpToMyRanking")}
          >
            <span>{t("rankingsRail.pager.myRank")}</span>
          </button>
        )}
        <button
          className="Jump-pagerButton"
      ...
      <div className="Jump-searchNavigation" aria-hidden={!searchActive}>
        <div className="Jump-searchNavigationContent">
          ...
          >
            <ArrowUpIcon />
            <span>{t("rankingsRail.pager.previousPerson")}</span>
          </button>
          <button
            className="Jump-searchNavigationButton"
            ...
            disabled={!searchActive}
          >
            <span>{t("rankingsRail.pager.nextPerson")}</span>
            <ArrowDownIcon />
          </button>
        </div>
      </div>
...
export function ListBrowseControlsRail({
  query,
  onQueryChange,
}: {
  query: string;
  ...
  return (
    <RankingsRail className="Jump--listBrowse" direction="up">
      <div className="findBar findBar--rail" data-open="true" role="search">
        <span className="listBrowseSearchIcon" aria-hidden="true">
          <SearchIcon />
        </span>
        <input
          className="findInput"
          ...
          aria-label={t("rankingsRail.search.publicListsLabel")}
          placeholder={t("rankingsRail.search.publicLists")}
          autoFocus
        />
      </div>
    </RankingsRail>
...
export function ListBrowsePagerRail({
  onJumpUp,
  onJumpDown,
}: {
  onJumpUp: () => void;
  ...
  return (
    <RankingsRail className="Jump--pager" direction="down">
      <div className="Jump-pagerActions">
        <button className="Jump-pagerButton" onClick={onJumpUp} type="button">
          <span>{t("rankingsRail.pager.jumpToTop")}</span>
          <ArrowUpIcon />
        </button>
        <button className="Jump-pagerButton" onClick={onJumpDown} type="button">
          <ArrowDownIcon />
          <span>{t("rankingsRail.pager.jumpToEnd")}</span>
        </button>
      </div>
    </RankingsRail>
  );
