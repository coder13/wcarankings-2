...
test("renders paired pager actions with useful labels", () => {
  const markup = renderToStaticMarkup(<JumpControlsVisibility visible><RankingsPagerRail navigation={{ currentPosition: 100, total: 10_000, onJumpUp: () => undefined, onJumpDown: () => undefined }} search={{ active: false, onPrevious: () => undefined, onNext: () => undefined }} /></JumpControlsVisibility>);
...
test("disables pager actions while a jump is settling", () => {
  const markup = renderToStaticMarkup(<RankingsPagerRail navigation={{ busy: true, currentPosition: 5_001, total: 10_000, onJumpUp: () => undefined, onJumpDown: () => undefined }} search={{ active: false, onPrevious: () => undefined, onNext: () => undefined }} />);
  assert.match(markup, /disabled=""/);
});

test("renders the ranking settings and search in one rail", () => {
  const markup = renderToStaticMarkup(<RankingsControlsRail controls={{ event: WCA_EVENTS[0], onEventChange: () => undefined, rankingType: "single", onRankingTypeChange: () => undefined, gender: [], onGenderChange: () => undefined, regions, regionSelection: { scope: "world", regionId: "" }, onRegionChange: () => undefined, compactResultType: false }} search={{ findOpen: false, findQuery: "", findError: "", findLoading: false, findPending: false, findMatches: [], findIndex: 0, onSearchOpen: () => undefined, onSearchClose: () => undefined, onSearchQueryChange: () => undefined, onSearchCycle: () => undefined }} />);
...
test("shows previous and next person actions while searching", () => {
  const markup = renderToStaticMarkup(<RankingsPagerRail navigation={{ currentPosition: 100, total: 10_000, onJumpUp: () => undefined, onJumpDown: () => undefined }} search={{ active: true, onPrevious: () => undefined, onNext: () => undefined }} />);
