...
type SingleExpandedVirtualRowOptions = {
  totalRows: number;
  rowHeight: number;
...
export function useSingleExpandedVirtualRow({
  ...
  duration,
  ease,
}: SingleExpandedVirtualRowOptions) {
  ...
  const rowSize = useCallback(
    (index: number) => {
      ...
      if (expanding?.index === index) {
        extraHeight += expanding.height;
      } else if (expandedIndex === index) {
        extraHeight += expandedExtraHeight;
      }
      if (collapsing?.index === index) {
        extraHeight += collapsing.height;
      }
  ...
  const offsetForIndex = useCallback(
    (requestedIndex: number) => {
      ...
      if (expanding && expanding.index < index) {
        offset += expanding.height;
      } else if (expandedIndex !== null && expandedIndex < index) {
        offset += expandedExtraHeight;
      }
      if (collapsing && collapsing.index < index) {
        offset += collapsing.height;
      }
  ...
  const toggle = useCallback(
    (requestedIndex: number, controls: RowTransitionControls) => {
      ...
      const expanding = nextExpandedIndex === null
        ? null
        : {
          index: nextExpandedIndex,
          height: openingStartHeight,
        };
      const collapsing = closingIndex === null
        ? null
        : {
          index: closingIndex,
          height: closingStartHeight,
        };
  ...
  return useMemo(
    ...
    [expandedIndex, finish, indexAtOffset, offsetForIndex, rowSize, toggle],
  );
}
