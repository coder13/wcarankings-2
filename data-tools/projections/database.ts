...
export const INDEXES = [
  ...
  ["results", "idx_results_average_country_best", "(`event_id`, `person_country_id`, `average`, `id`)", "event_id,average,id"],
];
...
export async function ensureWcaPersonLookupIndex(connection) {
  await ensureIndexes(connection, INDEXES.filter(([table, name]) => table === "persons" && name === "idx_persons_wca_sub"));
}
