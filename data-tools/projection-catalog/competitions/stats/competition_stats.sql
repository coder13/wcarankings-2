...
WITH competitor_counts AS (
  ...
  LEFT JOIN competitor_counts
    ON competitor_counts.competition_id = comp.id
)
SELECT competition_values.*,
  CASE WHEN competitor_count > 0 THEN
    DENSE_RANK() OVER (
      PARTITION BY competitor_count > 0
  ...
  CASE WHEN competitor_count > 0 THEN
    ROW_NUMBER() OVER (
      PARTITION BY competitor_count > 0
      ORDER BY competitor_count DESC, start_date, competition_id
    )
  ...
  CASE
    ...
    THEN ROW_NUMBER() OVER (
      ...
      ORDER BY latitude DESC, start_date, competition_id
    )
  ...
  CASE
    ...
    THEN ROW_NUMBER() OVER (
      ...
      ORDER BY latitude, start_date, competition_id
    )
  END AS southernmost_position
FROM competition_values;
