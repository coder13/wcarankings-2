...
WITH aggregates AS (
  SELECT
    ...
    MIN(
      CASE WHEN result.average > 0 THEN
        CAST(
          CONCAT(
  ...
  SELECT
    ...
    CASE
      WHEN fastest_single_key IS NOT NULL
      THEN FLOOR(fastest_single_key / 10000000000)
    END AS fastest_single,
    CASE
      WHEN fastest_single_key IS NOT NULL
      THEN MOD(fastest_single_key, 10000000000)
    END AS fastest_single_result_id,
    CASE
      WHEN fastest_average_key IS NOT NULL
      THEN FLOOR(fastest_average_key / 10000000000)
    END AS fastest_average,
    CASE
      WHEN fastest_average_key IS NOT NULL
      THEN MOD(fastest_average_key, 10000000000)
    END AS fastest_average_result_id,
    podiums.podium_score
  FROM aggregates aggregate
  LEFT JOIN podiums
    ON podiums.competition_id = aggregate.competition_id
...
SELECT
  ...
  CASE WHEN fastest_single IS NOT NULL THEN
    RANK() OVER (
      PARTITION BY event_id, fastest_single IS NULL
      ORDER BY fastest_single
    )
  END AS fastest_single_rank,
  CASE WHEN fastest_single IS NOT NULL THEN
    ROW_NUMBER() OVER (
      PARTITION BY event_id, fastest_single IS NULL
      ORDER BY fastest_single, start_date, competition_id
    )
  END AS fastest_single_position,
  CASE WHEN fastest_average IS NOT NULL THEN
    RANK() OVER (
      PARTITION BY event_id, fastest_average IS NULL
      ORDER BY fastest_average
    )
  END AS fastest_average_rank,
  CASE WHEN fastest_average IS NOT NULL THEN
    ROW_NUMBER() OVER (
      PARTITION BY event_id, fastest_average IS NULL
      ORDER BY fastest_average, start_date, competition_id
    )
  END AS fastest_average_position,
  CASE WHEN podium_score IS NOT NULL THEN
    RANK() OVER (
      PARTITION BY event_id, podium_score IS NULL
      ORDER BY podium_score
    )
  END AS podium_rank,
  CASE WHEN podium_score IS NOT NULL THEN
    ROW_NUMBER() OVER (
      PARTITION BY event_id, podium_score IS NULL
      ORDER BY podium_score, start_date, competition_id
    )
...
ALTER TABLE competition_event_stats
  ...
  ADD INDEX idx_competition_event_fastest_single (
    event_id, fastest_single_position
  ),
  ADD INDEX idx_competition_event_fastest_average (
    event_id, fastest_average_position
  ),
  ADD INDEX idx_competition_event_podium (
    event_id, podium_position
  );
