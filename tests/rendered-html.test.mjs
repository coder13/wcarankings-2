...
test("uses the copied WCA Rankings visual language", async () => {
  ...
  assert.match(globalCss, /\.stickyRankingsRail > \.RankingsRailTransition::before,\s*\.JumpControlsVisibility:has\(\.Jump\[data-direction="down"\]\) > \.RankingsRailTransition::before\s*\{[\s\S]*width:\s*calc\(100vw - 2em\);[\s\S]*box-shadow:[\s\S]*18px 12px[\s\S]*var\(--surface\) 75%[\s\S]*48px 28px[\s\S]*var\(--surface\) 34%/);
