...
test("normal rankings retain separate historical country and continent bests", async () => {
  const [
    ...
    listRankings,
    fixture,
    resultAttemptsMigration,
    personLookupMigration,
  ] = await Promise.all([
    readFile(
      new URL(
    ...
    readFile(
      new URL("tests/fixtures/regional-ranking-history.sql", root),
      "utf8",
    ),
    readFile(
      new URL("migrations/mysql/results/V8__result_attempts_lookup.sql", root),
      "utf8",
    ),
    readFile(
      new URL("migrations/mysql/app/V13__person_ranking_lookup.sql", root),
      "utf8",
    ),
  ]);
  ...
  assert.match(
    listRankings,
    /let rankingColumn = "world_rank";[\s\S]*input\.region\.scope === "continent"/,
  );
