...
type FocusRankings = Pick<
  ...
  "expandedIndex" | "toggleExpanded" | "expandIndex" | "jumpToIndex" | "loading"
>;
...
export function useRankingFocus({
  ...
  const focusWcaId = useCallback(
    (wcaId: string, animate = true) => {
  ...
  const focusMyRanking = useCallback(
    (wcaId: string) => {
  ...
  const updateFocusedPerson = useCallback(
