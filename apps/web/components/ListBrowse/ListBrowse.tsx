...
import {
  ListBrowseControlsRail,
  ListBrowsePagerRail,
} from "@/components/RankingsRail/RankingsRail";
...
export function ListBrowse({
  ...
  const [lists, setLists] = useState<PublicListSummary[] | null>(
    initialLists ?? null,
  );
  ...
  if (loadState === "loading") {
    listContent = (
      <ol
        className="listBrowseList listBrowseSkeleton"
        aria-label="Loading public lists"
        aria-busy="true"
      >
    ...
    listContent = (
      <p className="listBrowseEmpty" role="alert">
        Could not load public lists. Please refresh and try again.
      </p>
    );
  } else if (filtered.length) {
    listContent = (
      <ol
        className="listBrowseList"
    ...
    <div className="app">
      <AppHeader
        ...
        onSubjectChange={(value) => {
          if (value !== "lists") router.push(navigationPath(value));
        }}
      />
      <div className="stickyRankingsRail">
        <ListBrowseControlsRail query={query} onQueryChange={setQuery} />
      </div>
