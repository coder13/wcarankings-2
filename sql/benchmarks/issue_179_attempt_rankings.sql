...
ALTER TABLE benchmark_solve_facts
  ...
  ADD INDEX idx_benchmark_solve_global (gender, event_id, solve_value, result_id, attempt_number),
  ADD INDEX idx_benchmark_solve_person (person_id, event_id, solve_value, result_id, attempt_number);
...
SELECT
  ...
  RANK() OVER (
    PARTITION BY gender, event_id, country_id
    ORDER BY solve_value
  ) AS country_rank,
  ROW_NUMBER() OVER (
    PARTITION BY gender, event_id, country_id
...
ALTER TABLE benchmark_issue_179_single_rankings
  ...
  ADD INDEX idx_benchmark_issue179_continent (gender, event_id, continent_id, continent_position),
