...
WITH
  ...
  first_weeks AS (
    SELECT
      current_values.scope,
      current_values.event_id,
      current_values.region_id,
      MIN(scoped_results.week_start) AS first_week
    FROM
      current_values
      INNER JOIN scoped_results ON scoped_results.scope = current_values.scope
  ...
  streaks AS (
    SELECT
      first_weeks.scope,
      first_weeks.event_id,
      first_weeks.region_id,
      FLOOR(
        DATEDIFF(latest_week.week_start, first_weeks.first_week) / 7
      ) + 1 AS streak_weeks
    FROM
      first_weeks
  ...
  holders AS (
    SELECT DISTINCT
      scoped_results.event_id,
      scoped_results.person_id,
      streaks.scope,
      streaks.region_id,
      streaks.streak_weeks
    FROM
      scoped_results
      INNER JOIN current_values ON current_values.scope = scoped_results.scope
      ...
      INNER JOIN streaks ON streaks.scope = scoped_results.scope
...
SELECT
  ...
  MAX(
    CASE
      WHEN scope = 'world' THEN streak_weeks
    END
  ) AS world_record_streak_weeks,
  MAX(
    CASE
      WHEN scope = 'continent' THEN streak_weeks
    END
  ) AS continent_record_streak_weeks,
  MAX(
    CASE
      WHEN scope = 'country' THEN streak_weeks
    END
  ) AS country_record_streak_weeks
FROM
  holders
GROUP BY
  event_id,
