...
async function queryGenderPage(input: QueryInput) {
  ...
  const result = await query<RankingRow>(
    ...
    [...values, input.locate ? 1 : input.search ? input.searchLimit : input.limit + 1],
  );
...
async function queryPersonMetric(input: QueryInput) {
  ...
  const end = await query<{ position: number }>(personMetricEndQuery(positionColumn, gender.sql), [
    metricResultType,
    input.scope,
    input.regionId,
    ...gender.values,
...
async function queryFilteredPersonMetric(
  ...
  const entries = rows
    .slice(0, input.locate ? 1 : search ? rows.length : input.limit)
    .map(personMetricEntry);
  if (input.locate)
    return {
      data: { located: entries[0] ?? null },
