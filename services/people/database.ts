...
export async function fetchPersonSearchRowsFromDatabase(input: PersonSearchDatabaseInput) {
  ...
  try {
    ...
    const people = await query<PersonSearchRow>(personSearchRowsQuery(input, false), values);
    if (!people.rows.length) return people;
    const counts = await query<{ person_id: string; competition_count: number }>(
    ...
    const countsByPersonId = new Map(
      counts.rows.map((count) => [count.person_id, Number(count.competition_count)]),
    );
    return {
      ...people,
