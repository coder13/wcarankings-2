...
export async function getList(request: Request, context: ListRouteContext) {
  ...
  const [list, user] = await Promise.all([resolveList(listId), getAuthUser(request)]);
...
export async function updateUserList(request: Request, context: ListRouteContext) {
  assertSameOrigin(request);
  const user = await requireAuthUser(request);
  const body = await readJsonObject(request);
  const { listId } = await context.params;
...
export async function deleteUserList(request: Request, context: ListRouteContext) {
  assertSameOrigin(request);
  const user = await requireAuthUser(request);
  const { listId } = await context.params;
...
export async function getListMembers(request: Request, context: ListRouteContext) {
  const { listId } = await context.params;
  const [list, user] = await Promise.all([resolveList(listId), getAuthUser(request)]);
  assertCanViewList(list, user);
...
export async function removeListMemberById(request: Request, context: ListMemberRouteContext) {
  assertSameOrigin(request);
  const user = await requireAuthUser(request);
...
export async function removeCurrentUserFromList(request: Request, context: ListRouteContext) {
  assertSameOrigin(request);
  const user = await requireAuthUser(request);
  const { listId } = await context.params;
...
export async function getListRankings(request: Request, context: ListRouteContext) {
...
export async function getMembershipRequests(request: Request, context: ListRouteContext) {
  const user = await requireAuthUser(request);
  const { listId } = await context.params;
...
export async function createMembershipRequest(request: Request, context: ListRouteContext) {
  assertSameOrigin(request);
  const user = await requireAuthUser(request);
  const { listId } = await context.params;
...
export async function decideMembershipRequestForUser(
  ...
  const result = await decideMembershipRequest(user, listId, requestId, body.decision);
  return privateResponse({ status: body.decision, ...result });
}

export async function withListErrors(operation: () => Promise<Response>): Promise<Response> {
  try {
