...
export function listLookupQuery(input: ListLookupQueryInput) {
  return sqlFragment`
    ...
    WHERE
      ${input.byPublicId ? "l.public_id = ?" : "l.system_alias = ?"}
      AND l.deleted_at IS NULL
    LIMIT
      1${input.forUpdate ? " FOR UPDATE" : ""}
  `;
}
...
export function personIdsForListQuery(count: number) {
  return sqlFragment`
    ...
    WHERE
      sub_id = 1
      AND wca_id IN (${placeholders(count)})
  `;
}
...
export function insertBulkListMembersQuery(count: number) {
  return sqlFragment`
    INSERT INTO
      list_members (list_id, person_id, added_by_user_id, source)
    VALUES
      ${Array.from({ length: count }, () => "(?, ?, ?, 'bulk_import')").join(
        ",",
      )}
  `;
}
...
export function listMembersQuery() {
  return sqlFragment`
    ...
    FROM
      list_members AS member
      LEFT JOIN persons AS person ON person.wca_id = member.person_id
      AND person.sub_id = 1
    WHERE
      member.list_id = ?
...
export function blockedPersonIdsQuery(count: number) {
  return sqlFragment`
    ...
    FROM
      app_users AS user
    WHERE
      user.wca_id IN (${placeholders(count)})
      AND user.allow_list_inclusion = FALSE FOR
    UPDATE
  `;
}
export function excludedPersonIdsQuery(count: number) {
  return sqlFragment`
    ...
    WHERE
      list_id = ?
      AND person_id IN (${placeholders(count)}) FOR
    UPDATE
  `;
}
...
export function insertListExclusionQuery() {
  return sqlFragment`
    INSERT INTO
      list_exclusions (list_id, person_id)
    VALUES
      (?, ?)
...
export function membershipRequestsQuery() {
  return sqlFragment`
    SELECT
      ...
      request.resolved_at
    FROM
      list_membership_requests AS request
...
export function removeUserFromListsQuery() {
  return sqlFragment`
    ...
    SET
      list_record.member_count = GREATEST(
        0,
        list_record.member_count - removed.removed_count
      ),
...
export function listRegionsQuery() {
  return sqlFragment`
    ...
    FROM
      list_members AS member
      JOIN persons AS person ON person.wca_id = member.person_id
      AND person.sub_id = 1
...
export function dynamicListRegionsQuery(count: number) {
  return sqlFragment`
    ...
    WHERE
      person.sub_id = 1
      AND person.wca_id IN (${placeholders(count)})
    ORDER BY
      country.name,
      country.id
  `;
