...
export function listActivityQuery() {
  return sql`INSERT INTO list_activity_events (list_id, actor_user_id, event_type, person_id, event_data) VALUES (?, ?, ?, ?, ?)`;
}
export function createListQuery() {
  return sql`INSERT INTO lists (kind, public_id, owner_user_id, name, slug, description, visibility, join_policy) VALUES ('user', ?, ?, ?, ?, ?, ?, ?)`;
}
...
export function optedOutPersonIdsQuery(count: number) {
  return sql`SELECT wca_id FROM app_users WHERE allow_list_inclusion = FALSE AND wca_id IN (${placeholders(count)})`;
}
export function insertBulkListMembersQuery(count: number) {
  return sql`INSERT INTO list_members (list_id, person_id, added_by_user_id, source) VALUES ${Array.from({ length: count }, () => "(?, ?, ?, 'bulk_import')").join(",")}`;
}
...
export function cloneListQuery() {
  return sql`INSERT INTO lists (kind, public_id, owner_user_id, name, slug, description, visibility, join_policy) VALUES ('user', ?, ?, ?, ?, ?, 'private', 'closed')`;
}
export function cloneListMembersQuery() {
  return sql`INSERT INTO list_members (list_id, person_id, added_by_user_id, source) SELECT ?, person_id, ?, 'bulk_import' FROM list_members WHERE list_id = ?`;
}
...
export function ownedListsQuery() {
  return sql`SELECT ${LIST_COLUMNS} FROM lists AS l LEFT JOIN app_users AS owner ON owner.id = l.owner_user_id WHERE l.owner_user_id = ? AND l.deleted_at IS NULL ORDER BY l.updated_at DESC, l.id DESC`;
}
export function publicListsQuery() {
  return sql`SELECT ${LIST_COLUMNS} FROM lists AS l LEFT JOIN app_users AS owner ON owner.id = l.owner_user_id WHERE l.visibility = 'public' AND l.deleted_at IS NULL ORDER BY l.kind = 'system' DESC, l.name ASC, l.id ASC`;
}
export function containingUserListsQuery() {
  return sql`SELECT ${LIST_COLUMNS} FROM list_members AS member JOIN lists AS l ON l.id = member.list_id LEFT JOIN app_users AS owner ON owner.id = l.owner_user_id WHERE member.person_id = ? AND l.deleted_at IS NULL ORDER BY l.updated_at DESC, l.id DESC`;
}
...
export function listMembersQuery() {
  return sql`SELECT member.person_id, person.name AS person_name, person.country_id, member.source, member.created_at FROM list_members AS member LEFT JOIN persons AS person ON person.wca_id = member.person_id AND person.sub_id = 1 WHERE member.list_id = ? AND member.person_id > ? ORDER BY member.person_id LIMIT ?`;
}
export function validPersonIdsQuery(count: number) {
  return sql`SELECT wca_id FROM persons WHERE sub_id = 1 AND wca_id IN (${placeholders(count)})`;
}
export function blockedPersonIdsQuery(count: number) {
  return sql`SELECT user.wca_id FROM app_users AS user WHERE user.wca_id IN (${placeholders(count)}) AND user.allow_list_inclusion = FALSE FOR UPDATE`;
}
export function excludedPersonIdsQuery(count: number) {
  return sql`SELECT person_id FROM list_exclusions WHERE list_id = ? AND person_id IN (${placeholders(count)}) FOR UPDATE`;
}
export function existingListMemberIdsQuery(count: number) {
  return sql`SELECT person_id FROM list_members WHERE list_id = ? AND person_id IN (${placeholders(count)}) FOR UPDATE`;
}
export function insertListMembersQuery(count: number) {
  return sql`INSERT INTO list_members (list_id, person_id, added_by_user_id, source) VALUES ${Array.from({ length: count }, () => "(?, ?, ?, ?)").join(",")}`;
}
...
export function insertSelfRequestMemberQuery() {
  return sql`INSERT IGNORE INTO list_members (list_id, person_id, added_by_user_id, source) VALUES (?, ?, ?, 'self_request')`;
}
