...
if (!content.includes("      - name: Exercise bulk projection table transfer\n")) {
  const marker = "      - name: Exercise legacy Flyway baseline history split\n";
  const step = `      - name: Exercise bulk projection table transfer
        run: |
          set -euo pipefail
...
`;
  if (!content.includes(marker)) throw new Error("Could not locate PR integration insertion point");
  content = content.replace(marker, `${step}${marker}`);
}
