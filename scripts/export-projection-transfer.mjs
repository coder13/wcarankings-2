...
if (!metadata.group || !Array.isArray(metadata.tables) || metadata.tables.length === 0) {
...
const workDirectory = await mkdtemp(join(tmpdir(), `projection-transfer-${safeGroup}-`));
...
try {
  ...
  `;
  await run("docker", [
    "compose", "exec", "-T", "db", "sh", "-ceu", exportScript, "sh",
    containerDirectory, ...metadata.tables,
  ]);
  await run("docker", ["compose", "cp", `db:${containerDirectory}/.`, copiedDirectory]);
  await run("sh", ["-ceu", "tar -C \"$1\" -cf - . | gzip -1 > \"$2\"", "sh", copiedDirectory, outputPath]);
