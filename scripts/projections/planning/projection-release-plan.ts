...
async function main(): Promise<void> {
  ...
  const sourceManifest = objectOrUndefined(
    await readJsonFile(argumentValue("source-manifest-file")),
  );
  const previousSourceManifest = objectOrUndefined(
    await readJsonFile(argumentValue("previous-source-manifest-file")),
  );
  ...
  const plan = await projectionReleasePlan({
    ...
    forceRebuild,
    sourceManifest:
      sourceManifest && Object.keys(sourceManifest).length > 0
        ? (sourceManifest as unknown as SourceManifest)
        : undefined,
    previousSourceManifest:
      previousSourceManifest && Object.keys(previousSourceManifest).length > 0
        ? (previousSourceManifest as unknown as SourceManifest)
        : undefined,
    currentYear: argumentValue("current-year")
      ? Number(argumentValue("current-year"))
      : undefined,
  });
  process.stdout.write(`${JSON.stringify(plan, null, 2)}\n`);
}
