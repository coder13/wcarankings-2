...
export async function createProjectionReleaseManifest({
  ...
  for (const group of groups) {
    ...
    const artifactFingerprint =
      desired?.artifactFingerprint ?? desired?.fingerprint;
...
export async function verifyProjectionReleaseManifest({
  ...
  if (manifest.version !== 3)
    throw new Error(
      `Unsupported projection release manifest version: ${manifest.version}`,
    );
  if (
  ...
  if (manifest.raw) {
    if (
      !manifest.raw.file ||
      basename(manifest.raw.file) !== manifest.raw.file
    ) {
      throw new Error(
        "Projection release contains an invalid raw artifact path",
      );
    }
    const actual = await artifactMetadata(directory, manifest.raw.file);
    if (
      actual.sha256 !== manifest.raw.sha256 ||
      actual.bytes !== manifest.raw.bytes
    ) {
      throw new Error(
        `Projection release artifact ${manifest.raw.file} failed verification`,
      );
    }
  }
  for (const group of groups) {
    ...
    for (const artifact of [entry.archive, entry.metadata]) {
      if (!artifact?.file || basename(artifact.file) !== artifact.file) {
        throw new Error(
          `Projection release group ${group} contains an invalid artifact path`,
        );
      }
      ...
      if (
        ...
        throw new Error(
          `Projection release artifact ${artifact.file} failed verification`,
        );
      }
    }
    const transfer = JSON.parse(
      await readFile(join(directory, entry.metadata.file), "utf8"),
    );
    if (transfer.group !== group) {
      throw new Error(
...
async function cli() {
  ...
  if (command === "create") {
    ...
    const artifactDigests = argumentValue("artifact-digests-file")
      ? JSON.parse(
          await readFile(argumentValue("artifact-digests-file"), "utf8"),
        )
      : {};
