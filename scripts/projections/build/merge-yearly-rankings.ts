...
import {
  projectionSql,
  statements,
} from "../../../data-tools/projections/build/sql.ts";
...
export function quoteYears(years: readonly number[]): string {
  if (
    years.length === 0 ||
    years.some((year) => !Number.isInteger(year) || year < 1)
  ) {
...
export function dirtySql(sql: string, years: readonly number[]): string {
  const renamed = [...TABLES]
    ...
    .reduce(
      (value, table) => value.replaceAll(table, `${table}${SUFFIX}`),
      sql,
    );
  return renamed.replaceAll(
    "period_year > 0",
    `period_year IN (${quoteYears(years)})`,
  );
}
...
async function main(): Promise<void> {
  const years = argumentList("years")
    .map(Number)
    .sort((left, right) => left - right);
  quoteYears(years);
  const mysql = await import("mysql2/promise");
  const { databaseOptions } = await import("@wcarankings/database");
  const connection = await mysql.createConnection(databaseOptions());
  try {
    await connection.query("SET SESSION max_statement_time = 0");
    for (const table of TABLES)
      await connection.query(`DROP TABLE IF EXISTS \`${table}${SUFFIX}\``);
    const cohortSql = await projectionSql(
      "people/year-rankings/person_year_ranking_cohorts.sql",
    );
    const singleSql = await projectionSql(
      "people/year-rankings/person_year_rankings_single.sql",
    );
    const averageSql = await projectionSql(
    ...
    try {
      ...
      for (const table of TABLES.slice(1)) {
        await connection.query(
          `DELETE FROM \`${table}\` WHERE year IN (${yearList})`,
        );
        await connection.query(
          `INSERT INTO \`${table}\` SELECT * FROM \`${table}${SUFFIX}\``,
        );
      }
