...
import type { LiveResult, LiveResultsSnapshot, LiveResultSource } from "./types.ts";
...
async function fetchJson(url: string): Promise<unknown> {
  const response = await fetch(url, {
    headers: { Accept: "application/json", "User-Agent": "wcarankings-live-results-worker/1.0" },
...
function resultFromWcaLive(eventId: string, roundNumber: number, row: JsonObject): LiveResult {
...
export function normalizeWcaLiveResults(payload: unknown): LiveResultsSnapshot {
  if (!isObject(payload) || !Array.isArray(payload.events) || !Array.isArray(payload.persons)) {
  ...
  for (const event of payload.events) {
    ...
    for (const round of event.rounds) {
      ...
      for (const result of round.results) {
        ...
        results.push(resultFromWcaLive(eventId, roundNumber, { ...result, ...person }));
      }
    }
  }
  return { results };
...
export function canonicalSnapshot(snapshot: LiveResultsSnapshot): string {
  return JSON.stringify([...snapshot.results].sort((left, right) => left.sourceResultId.localeCompare(right.sourceResultId)));
}
...
async function fetchCubingChinaResults(remoteCompetitionId: string): Promise<LiveResultsSnapshot> {
  ...
  for (const endpoint of endpointCandidates) {
    try {
      ...
      if (Array.isArray(payload)) { competitors = payload; break; }
      if (isObject(payload) && Array.isArray(payload.persons)) {
        competitors = payload.persons.filter((person) => isObject(person) && (!isObject(person.registration) || person.registration.status === "accepted"));
        break;
      }
    } catch {
  ...
  if (!competitors?.length) throw new Error("Cubing China returned no accepted competitors.");
  const fetchOne = async (competitor: unknown): Promise<JsonObject[]> => {
    ...
    const number = asInteger(competitor.number) ?? asInteger(competitor.registrantId) ?? asInteger(user.registrantId);
    if (number === null) return [];
    const wcaId = asString(user.wcaId) ?? asString(user.wcaid) ?? "";
    const params = new URLSearchParams({ "user[number]": String(number), "user[wcaid]": wcaId });
    const payload = unwrap(await fetchJson(`${CUBING_CHINA_BASE_URL}/live/${alias}/userResults?${params}`));
    if (!Array.isArray(payload)) return [];
    return payload.filter((row) => isObject(row) && row.t === "r").map((row) => ({
      resultId: row.i, eventId: row.e, roundId: row.r, formatId: row.f, best: row.b,
      average: row.a, attempts: row.v, wcaId, name: user.name,
    }));
  };
  const rows: JsonObject[][] = [];
  for (let index = 0; index < competitors.length; index += 8) {
    rows.push(...await Promise.all(competitors.slice(index, index + 8).map(fetchOne)));
  }
  return normalizeCubingChinaResults({ results: rows.flat() });
}

export async function fetchLiveResults(source: LiveResultSource, remoteCompetitionId: string): Promise<LiveResultsSnapshot> {
  if (source === "wca-live") {
    return normalizeWcaLiveResults(await fetchJson(`${WCA_LIVE_BASE_URL}/api/competitions/${encodeURIComponent(remoteCompetitionId)}/results`));
  }
