...
export async function enqueueListRankingRebuild(
  ...
  await connection.execute(
    `INSERT INTO list_ranking_rebuild_jobs
      (list_id, target_key, membership_version, rankings_data_version, priority, available_at, lease_token, leased_until, attempts, last_error)
     ...
     ON DUPLICATE KEY UPDATE
       ...
       last_error = NULL`,
    [list.id, targetKey, list.membershipVersion, dataVersion, priority],
  );
}
...
export async function enqueueAllListRankingRebuilds(connection: PoolConnection) {
  ...
  await connection.execute(
    `INSERT INTO list_ranking_rebuild_jobs
      (list_id, target_key, membership_version, rankings_data_version, priority, available_at, lease_token, leased_until, attempts, last_error)
     SELECT list.id, CONCAT('list:', list.id), list.membership_version, ?, IF(list.kind = 'system', ?, ?), CURRENT_TIMESTAMP(6), NULL, NULL, 0, NULL
...
export async function raiseListRankingRebuildPriority(list: ListTarget) {
  ...
  await query(
    `INSERT INTO list_ranking_rebuild_jobs
      (list_id, target_key, membership_version, rankings_data_version, priority, available_at)
     VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP(6))
     ON DUPLICATE KEY UPDATE
       list_id = VALUES(list_id), membership_version = VALUES(membership_version), rankings_data_version = VALUES(rankings_data_version),
       priority = GREATEST(priority, VALUES(priority)), available_at = LEAST(available_at, VALUES(available_at))`,
    [list.id, targetKey, list.membershipVersion, dataVersion, list.kind === "system" ? LIST_RANKING_PRIORITY.system : LIST_RANKING_PRIORITY.active],
  );
}

export async function ensureDynamicListRankingTarget(personIds: string[]) {
  ...
  const ready = await query<{ id: number }>(
    `SELECT id FROM list_ranking_cache_versions
     WHERE target_key = ? AND membership_version = 1 AND rankings_data_version = ? AND status = 'ready'
     LIMIT 1`,
    [targetKey, dataVersion],
  );
    ...
    await query(
      `INSERT INTO list_ranking_rebuild_jobs
       ...
       ON DUPLICATE KEY UPDATE
         membership_version = VALUES(membership_version), rankings_data_version = VALUES(rankings_data_version),
         priority = GREATEST(priority, VALUES(priority)), available_at = LEAST(available_at, VALUES(available_at))`,
      [targetKey, dataVersion, LIST_RANKING_PRIORITY.active],
    );
  }
