...
export function parseListRankingInput(searchParams: URLSearchParams) {
  ...
  return { eventId, type, start, limit, search, locate, region, gender,
    membershipVersion: Number.isSafeInteger(requestedMembershipVersion) && requestedMembershipVersion > 0 ? requestedMembershipVersion : null,
...
async function loadCachedListRankings(list: ListSummary, input: ReturnType<typeof parseListRankingInput>) {
  const metadata = await getCurrentRankingsMetadata();
  const dataVersion = await query<{ value: string }>("SELECT value FROM export_metadata WHERE `key` = 'fetched_at' LIMIT 1");
  ...
  const source = rankingTable(input.type);
  const cache = await query<ListRankingRow & { cache_version_id: number }>(
    `SELECT entry.list_rank AS rank, entry.list_position AS sub_rank, scope.total_count AS total,
       entry.person_id, ranking.person_name, ranking.country_id, ranking.country_name, ranking.country_iso2,
       ranking.continent_id, entry.score AS best, ranking.competition_id, ranking.competition_name,
       ranking.is_world_record, ranking.is_continent_record, ranking.is_country_record, version.id AS cache_version_id
     FROM list_ranking_cache_versions version
     JOIN list_ranking_cache_scopes scope ON scope.cache_version_id = version.id AND scope.event_id = ? AND scope.result_type = ?
     JOIN list_ranking_cache_entries entry ON entry.cache_version_id = version.id AND entry.event_id = scope.event_id AND entry.result_type = scope.result_type
     JOIN ${source} ranking ON ranking.event_id = entry.event_id AND ranking.person_id = entry.person_id
     WHERE version.list_id = ? AND version.membership_version = ? AND version.rankings_data_version = ? AND version.status = 'ready'
       AND entry.list_position > ?
     ORDER BY entry.list_position LIMIT ?`,
    [input.eventId, input.type, list.id, membershipVersion, rankingsDataVersion, input.start, input.limit + 1],
    { rankingStatementTimeout: true },
  );
  if (!cache.rows.length) {
    const exists = await query<{ id: number }>(
      `SELECT id FROM list_ranking_cache_versions WHERE list_id = ? AND membership_version = ? AND rankings_data_version = ? AND status = 'ready' LIMIT 1`,
  ...
  return {
    entries: rows.map((row) => ({
      rank: Number(row.rank), subRank: Number(row.sub_rank), personId: row.person_id, personName: row.person_name,
      countryId: row.country_id, countryName: row.country_name, countryIso2: row.country_iso2,
      continentId: row.continent_id, best: Number(row.best), competitionId: row.competition_id,
      competitionName: row.competition_name,
      recordBadges: getRecordBadges({ isWorldRecord: Number(row.is_world_record) === 1, isContinentRecord: Number(row.is_continent_record) === 1, isCountryRecord: Number(row.is_country_record) === 1, continentId: row.continent_id }),
    })),
...
export async function loadListRankings(
  ...
  if (isDefaultScope) {
    ...
    if (cached) return {
      list: { publicId: list.publicId, systemAlias: list.systemAlias, name: list.name, kind: list.kind, memberCount: list.memberCount, membershipVersion: list.membershipVersion },
      ...cached,
    };
  ...
  const rankings = await loadScopedRankings({
    ...
    values: [list.id],
  }, searchParams, isDefaultScope ? 100 : 0);
  const fallbackDataVersion = isDefaultScope
    ? (await query<{ value: string }>("SELECT value FROM export_metadata WHERE `key` = 'fetched_at' LIMIT 1")).rows[0]?.value ?? null
    : null;
  return {
    list: {
    ...
    ...rankings,
    cacheMembershipVersion: list.membershipVersion,
