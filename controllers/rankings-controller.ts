...
function buildListRankingResponse(rankings: Awaited<ReturnType<typeof loadListRankings>>, searchParams: URLSearchParams) {
  ...
  return {
    ...
    previousPageStart: inputStart > 0 ? Math.max(0, inputStart - Number(searchParams.get("limit") || 50)) + 1 : null,
...
async function fetchSavedListRankings(request: Request, input: RankingsQuery) {
  ...
  return { list, data: buildListRankingResponse(await loadListRankings(list, input.searchParams), input.searchParams) };
}
...
function buildGlobalRankingsResponse(input: RankingsQuery, loaded: Awaited<ReturnType<typeof fetchGlobalRankings>>) {
  ...
  console.info(JSON.stringify({ operation: "rankings", eventId: input.eventId, result: input.type, region: input.scope, status: 200, timings: { validation_ms: validationMs, cache_ms: cacheMs, db_queue_ms: queueMs, db_ms: statementMs, serialization_ms: 0, total_ms: totalMs }, query_count: result.queryCount, returned_rows: result.returnedRows, cache: result.cacheOutcome, data_version: result.dataVersion }));
...
function buildRankingsErrorResponse(input: RankingsQuery | null, startedAt: number, error: unknown) {
  ...
  console.error(JSON.stringify({ operation: "rankings", eventId: input?.eventId ?? "333", result: input?.type ?? "single", region: input?.scope ?? "world", status, timings: { total_ms: performance.now() - startedAt }, query_count: 0, returned_rows: 0, cache: "bypass", data_version: null, error: error instanceof Error ? error.name : "unknown" }));
...
export async function handleRankingsRequest(request: Request) {
  ...
  try {
    ...
    if (input.listId && input.hasDynamicList) throw new ApiInputError("Choose either a saved list or dynamic WCA IDs.");
    if (input.listId) {
      const loaded = await fetchSavedListRankings(request, input);
      return buildApiJsonResponse(loaded.data, { headers: { "Cache-Control": loaded.list.visibility === "public" ? "public, max-age=30, s-maxage=300, stale-while-revalidate=60" : "private, no-store" } });
    }
    if (input.hasDynamicList) {
      const loaded = await fetchDynamicListRankings(input);
      return buildApiJsonResponse(loaded.data, { headers: { "Cache-Control": "public, max-age=30, s-maxage=300, stale-while-revalidate=60" } });
    }
